...
class NotificationRepositoryImpl implements NotificationRepository {
  ...
  @override
  Future<List<IsarNotification>> getNotificationHistory() async {
    final list = await _isar.collection<IsarNotification>().where().findAll();
    list.sort((a, b) => b.receivedAt.compareTo(a.receivedAt));
    return list;
  }

  @override
  Future<void> saveNotification(IsarNotification notification) async {
    await _isar.writeTxn(() async {
      await _isar.collection<IsarNotification>().put(notification);
    });
  }
  ...
  Future<void> markAsRead(int id) async {
    await _isar.writeTxn(() async {
      final notification = await _isar.collection<IsarNotification>().get(id);
  ...
  Future<void> markAllAsRead() async {
    await _isar.writeTxn(() async {
      ...
      for (final n in unread) {
        n.isRead = true;
        await _isar.collection<IsarNotification>().put(n);
      }
    });
  }
  ...
  Future<void> deleteNotification(int id) async {
    await _isar.writeTxn(() async {
      await _isar.collection<IsarNotification>().delete(id);
    });
  }
  ...
  Future<void> clearAllNotifications() async {
    await _isar.writeTxn(() async {
      await _isar.collection<IsarNotification>().clear();
    });
  }

  @override
  Future<List<IsarNotification>> searchNotifications(String query) async {
    final list = await _isar.collection<IsarNotification>().where().findAll();
    if (query.isEmpty) {
      list.sort((a, b) => b.receivedAt.compareTo(a.receivedAt));
      return list;
